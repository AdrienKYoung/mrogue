import libtcodpy
from actions import item_actions
import player

table = {
    # Daggers
    'weapon_dagger': {
        'name'               : 'dagger',
        'category'           : 'weapon',
        'subtype'            : 'dagger',
        'damage_types'       : ['stabbing'],
        'char'               : '-',
        'color'              : libtcodpy.yellow,
        'type'               : 'item',
        'slot'               :'right hand',
        'description'        : 'A small double-edged knife. Deals triple damage to incapacitated targets',
        'stamina_cost'       : 6,
        'str_requirement'    : 6,
        'shred'              : 0,
        'accuracy'           : 5,
        'weapon_dice'        : '1d16',
        'str_dice'           : 1,
        'attack_delay'       : 12,
        'crit_bonus'         : 3.0
    },
    'weapon_messer': { #lit. german: 'knife'. no english period term
        'name'               : 'messer',
        'category'           : 'weapon',
        'damage_types'       : ['slashing'],
        'subtype'            : 'dagger',
        'char'               : '-',
        'color'              : libtcodpy.yellow,
        'type'               : 'item',
        'slot'               :'right hand',
        'description'        : 'A long knife, made for dueling',
        'stamina_cost'       : 8,
        'str_requirement'    : 8,
        'shred'              : 0,
        'accuracy'           : 5,
        'weapon_dice'        : '1d16',
        'str_dice'           : 2,
        'attack_delay'       : 14,
        'crit_bonus'         : 2.0
    },
    'weapon_katar': {
        'name'               : 'katar',
        'category'           : 'weapon',
        'damage_types'       : ['stabbing'],
        'subtype'            : 'dagger',
        'char'               : chr(19),
        'color'              : libtcodpy.yellow,
        'type'               : 'item',
        'slot'               :'both hands',
        'description'        : 'A pair of exotic hooded punching daggers.',
        'stamina_cost'       : 8,
        'str_requirement'    : 8,
        'shred'              : 1,
        'accuracy'           : 5,
        'weapon_dice'        : '1d30',
        'str_dice'           : 1,
        'attack_delay'       : 10,
        'crit_bonus'         : 3.0
    },
    'weapon_assassins_blade': {
        'name'               : "assassin's blade",
        'category'           : 'weapon',
        'damage_types'       : ['stabbing'],
        'subtype'            : 'dagger',
        'char'               : chr(19),
        'color'              : libtcodpy.yellow,
        'type'               : 'item',
        'slot'               : 'right hand',
        'description'        : 'A long thin blade with a handle of human bone, once wielded by a high-priced assassin.',
        'stamina_cost'       : 4,
        'str_requirement'    : 10,
        'pierce'             : 5,
        'shred'              : 0,
        'accuracy'           : 8,
        'weapon_dice'        : '1d36',
        'str_dice'           : 1,
        'attack_delay'       : 13,
        'crit_bonus'         : 5.0
    },
    # Swords
    'weapon_shortsword': {
        'name'               : 'shortsword',
        'category'           : 'weapon',
        'subtype'            : 'sword',
        'damage_types'       : ['slashing'],
        'char'               : '/',
        'color'              : libtcodpy.yellow,
        'type'               : 'item',
        'slot'               :'right hand',
        'description'        : 'A single handed cruciform sword',
        'stamina_cost'       : 10,
        'str_requirement'    : 9,
        'shred'              : 1,
        'accuracy'           : 3,
        'weapon_dice'        : '1d18',
        'str_dice'           : 1,
        'attack_delay'       : 11,
        'crit_bonus'         : 1.5
    },
    'weapon_longsword': {
        'name'               : 'longsword',
        'category'           : 'weapon',
        'subtype'            : 'sword',
        'damage_types'       : ['slashing'],
        'char'               : '/',
        'color'              : libtcodpy.yellow,
        'type'               : 'item',
        'slot'               :'right hand',
        'description'        : 'A hand-and-a-half cruciform sword',
        'stamina_cost'       : 10,
        'str_requirement'    : 10,
        'shred'              : 1,
        'accuracy'           : 1,
        'weapon_dice'        : '1d24',
        'str_dice'           : 2,
        'attack_delay'       : 14,
        'crit_bonus'         : 1.5
    },
    'weapon_rapier': {
        'name'               : 'rapier',
        'category'           : 'weapon',
        'subtype'            : 'sword',
        'damage_types'       : ['stabbing'],
        'char'               : '/',
        'color'              : libtcodpy.yellow,
        'type'               : 'item',
        'slot'               :'right hand',
        'description'        : 'A light, long, basket-hilted sword made for dueling',
        'stamina_cost'       : 6,
        'str_requirement'    : 8,
        'shred'              : 1,
        'accuracy'           : 3,
        'weapon_dice'        : '1d20',
        'str_dice'           : 2,
        'attack_delay'       : 12,
        'crit_bonus'         : 2.0
    },
    'weapon_greatsword': { #called a spadone or montante in historical circles, but no one will recognize that
        'name'               : 'greatsword',
        'category'           : 'weapon',
        'subtype'            : 'sword',
        'damage_types'       : ['slashing'],
        'char'               : '/',
        'color'              : libtcodpy.yellow,
        'type'               : 'item',
        'slot'               :'both hands',
        'description'        : 'A huge greatsword, made for fighting many foes at once.',
        'stamina_cost'       : 12,
        'str_requirement'    : 18,
        'shred'              : 2,
        'accuracy'           : 2,
        'weapon_dice'        : '1d36',
        'str_dice'           : 3,
        'attack_delay'       : 14,
        'crit_bonus'         : 1.5
    },
    'weapon_scimitar': {
        'name'               : 'scimitar',
        'category'           : 'weapon',
        'subtype'            : 'sword',
        'damage_types'       : ['slashing'],
        'char'               : '/',
        'color'              : libtcodpy.yellow,
        'type'               : 'item',
        'slot'               :'right hand',
        'description'        : 'A long curved sword with a wide blade from the arid lands of Sko Kall.',
        'stamina_cost'       : 16,
        'str_requirement'    : 18,
        'shred'              : 2,
        'accuracy'           : 0,
        'weapon_dice'        : '1d38',
        'str_dice'           : 2,
        'attack_delay'       : 16,
        'crit_bonus'         : 1.5
    },
    'weapon_blessed_blade': {
        'name'               : 'blessed blade',
        'category'           : 'weapon',
        'subtype'            : 'sword',
        'damage_types'       : ['slashing', 'radiance'],
        'char'               : '/',
        'color'              : libtcodpy.yellow,
        'type'               : 'item',
        'slot'               :'right hand',
        'description'        : 'A sacred weapon blessed with the essence of Radiance. May its wielder be a living '
                               'scourge to the unholy.',
        'stamina_cost'       : 12,
        'str_requirement'    : 15,
        'shred'              : 2,
        'accuracy'           : 0,
        'weapon_dice'        : '1d40',
        'str_dice'           : 2,
        'attack_delay'       : 12,
        'crit_bonus'         : 1.5,
        'on_hit'             : ['on_hit_judgement'],
    },
    'weapon_black_blade': {
        'name'               : 'black blade',
        'category'           : 'weapon',
        'subtype'            : 'sword',
        'damage_types'       : ['slashing', 'void'],
        'char'               : '/',
        'color'              : libtcodpy.yellow,
        'type'               : 'item',
        'slot'               :'right hand',
        'description'        : 'The Defiler, the blade that corrupted even angels.',
        'stamina_cost'       : 12,
        'str_requirement'    : 15,
        'shred'              : 2,
        'accuracy'           : 5,
        'weapon_dice'        : '1d60',
        'str_dice'           : 3,
        'attack_delay'       : 15,
        'crit_bonus'         : 1.5,
        'on_hit'             : ['on_hit_self_corruption'],
    },
    # Spears
    'weapon_shortspear': {
        'name'               : 'shortspear',
        'category'           : 'weapon',
        'subtype'            : 'polearm',
        'damage_types'       : ['stabbing'],
        'char'               : libtcodpy.CHAR_ARROW_N,
        'color'              : libtcodpy.yellow,
        'type'               : 'item',
        'slot'               : 'right hand',
        'description'        : 'A light one-handed pole with a sharpened tip. A primitive weapon, used for hunting.',
        'stamina_cost'       : 6,
        'str_requirement'    : 4,
        'pierce'             : 1,
        'shred'              : 0,
        'accuracy'           : 3,
        'weapon_dice'        : '1d12',
        'str_dice'           : 1,
        'attack_delay'       : 12,
        'crit_bonus'         : 1.5,
        'allowed_materials'  : ['wooden']
    },
    'weapon_spear': {
        'name'               : 'spear',
        'category'           : 'weapon',
        'subtype'            : 'polearm',
        'damage_types'       : ['stabbing'],
        'char'               : libtcodpy.CHAR_ARROW_N,
        'color'              : libtcodpy.yellow,
        'type'               : 'item',
        'slot'               :'right hand',
        'description'        : 'A light thrusting spear',
        'stamina_cost'       : 10,
        'str_requirement'    : 8,
        'ability'            : 'ability_thrust',
        'pierce'             : 1,
        'shred'              : 0,
        'accuracy'           : 3,
        'ctrl_attack'        : player.reach_attack,
        'ctrl_attack_desc'   : 'Reach-Attack - attack an enemy up to 2 spaces away in this direction.',
        'weapon_dice'        : '1d22',
        'str_dice'           : 1,
        'attack_delay'       : 14,
        'crit_bonus'         : 1.5
    },
    'weapon_partisan': {
        'name'               : 'partisan',
        'category'           : 'weapon',
        'subtype'            : 'polearm',
        'damage_types'       : ['stabbing'],
        'char'               : libtcodpy.CHAR_ARROW_N,
        'color'              : libtcodpy.yellow,
        'type'               : 'item',
        'slot'               :'right hand',
        'description'        : 'A one-handed spear with a winged blade for parrying and slicing.',
        'stamina_cost'       : 15,
        'str_requirement'    : 12,
        'ability'            : 'ability_thrust',
        'pierce'             : 1,
        'shred'              : 1,
        'accuracy'           : 1,
        'ctrl_attack'        : player.reach_attack,
        'ctrl_attack_desc'   : 'Reach-Attack - attack an enemy up to 2 spaces away in this direction.',
        'weapon_dice'        : '1d32',
        'str_dice'           : 1,
        'attack_delay'       : 17,
        'crit_bonus'         : 1.5
    },
    'weapon_halberd': {
        'name'               : 'halberd',
        'category'           : 'weapon',
        'subtype'            : 'polearm',
        'damage_types'       : ['slashing'],
        'char'               : libtcodpy.CHAR_ARROW_N,
        'color'              : libtcodpy.yellow,
        'type'               : 'item',
        'slot'               :'both hands',
        'description'        : 'A superior two-handed polearm with a spike, axe and hook on its head.',
        'stamina_cost'       : 18,
        'str_requirement'    : 16,
        'ability'            : 'ability_thrust_and_cleave',
        'pierce'             : 2,
        'shred'              : 0,
        'accuracy'           : 2,
        'ctrl_attack'        : player.reach_and_cleave_attack,
        'ctrl_attack_desc'   : 'Reach-Cleave-Attack - attack an enemy up to 2 spaces away in this direction and all '
                               'enemies adjacent to it. Costs 2x stamina.',
        'weapon_dice'        : '1d50',
        'str_dice'           : 2,
        'attack_delay'       : 14,
        'crit_bonus'         : 1.5
    },
    'weapon_scythe': {
        'name'               : 'scythe',
        'category'           : 'weapon',
        'subtype'            : 'polearm',
        'damage_types'       : ['slashing'],
        'char'               : libtcodpy.CHAR_ARROW_N,
        'color'              : libtcodpy.yellow,
        'type'               : 'item',
        'slot'               :'both hands',
        'description'        : 'A two-handed polearm with a long, arcing blade',
        'stamina_cost'       : 14,
        'str_requirement'    : 14,
        'ability'            : 'ability_thrust_and_cleave',
        'pierce'             : 3,
        'shred'              : 3,
        'accuracy'           : 2,
        'ctrl_attack'        : player.reach_and_cleave_attack,
        'ctrl_attack_desc'   : 'Reach-Cleave-Attack - attack an enemy up to 2 spaces away in this direction and all '
                               'enemies adjacent to it. Costs 2x stamina.',
        'weapon_dice'        : '1d64',
        'str_dice'           : 2,
        'attack_delay'       : 14,
        'crit_bonus'         : 1.5
    },
    # Axes
    'weapon_hatchet': {
        'name'               : 'hatchet',
        'category'           : 'weapon',
        'subtype'            : 'axe',
        'damage_types'       : ['slashing'],
        'char'               : 'p',
        'color'              : libtcodpy.yellow,
        'type'               : 'item',
        'slot'               : 'right hand',
        'description'        : 'A one-handed axe made for cutting wood.',
        'stamina_cost'       : 9,
        'str_requirement'    : 8,
        'shred'              : 1,
        'accuracy'           : 3,
        'ctrl_attack'        : player.cleave_attack,
        'ability'            : 'ability_cleave',
        'ctrl_attack_desc'   : 'Cleave - attack all adjacent enemies. Costs 2x stamina.',
        'weapon_dice'        : '1d18',
        'str_dice'           : 1,
        'attack_delay'       : 16,
        'crit_bonus'         : 1.5
    },
    'weapon_war_axe': {
        'name'               : 'war axe',
        'category'           : 'weapon',
        'subtype'            : 'axe',
        'damage_types'       : ['slashing'],
        'char'               : 'p',
        'color'              : libtcodpy.yellow,
        'type'               : 'item',
        'slot'               :'right hand',
        'description'        : 'A sturdy one-handed axe forged for battle.',
        'stamina_cost'       : 14,
        'str_requirement'    : 13,
        'shred'              : 1,
        'accuracy'           : 1,
        'ctrl_attack'        : player.cleave_attack,
        'ability'            : 'ability_cleave',
        'ctrl_attack_desc'   : 'Cleave - attack all adjacent enemies. Costs 2x stamina.',
        'weapon_dice'        : '1d24',
        'str_dice'           : 2,
        'attack_delay'       : 18,
        'crit_bonus'         : 1.5
    },
    'weapon_raider_axe': {
        'name'               : 'raider axe',
        'category'           : 'weapon',
        'subtype'            : 'axe',
        'damage_types'       : ['slashing'],
        'char'               : 'P',
        'color'              : libtcodpy.yellow,
        'type'               : 'item',
        'slot'               : 'both hands',
        'description'        : 'A massive, two handed axe made for smashing shields and armor.',
        'stamina_cost'       : 12,
        'str_requirement'    : 16,
        'shred'              : 3,
        'accuracy'           : 3,
        'ctrl_attack'        : player.cleave_attack,
        'ability'            : 'ability_cleave',
        'ctrl_attack_desc'   : 'Cleave - attack all adjacent enemies. Costs 2x stamina.',
        'weapon_dice'        : '1d36',
        'str_dice'           : 3,
        'attack_delay'       : 16,
        'crit_bonus'         : 1.5
    },
    'weapon_giant_axe': {
        'name'               : 'giant axe',
        'category'           : 'weapon',
        'subtype'            : 'axe',
        'damage_types'       : ['slashing'],
        'char'               : 'P',
        'color'              : libtcodpy.yellow,
        'type'               : 'item',
        'slot'               : 'both hands',
        'description'        : 'A colossal axe made for the hands of giants.',
        'stamina_cost'       : 22,
        'str_requirement'    : 30,
        'shred'              : 4,
        'accuracy'           : 3,
        'ctrl_attack'        : player.cleave_attack,
        'ability'            : 'ability_cleave',
        'ctrl_attack_desc'   : 'Cleave - attack all adjacent enemies. Costs 2x stamina.',
        'weapon_dice'        : '1d70',
        'str_dice'           : 6,
        'attack_delay'       : 50,
        'crit_bonus'         : 1.5
    },
    'weapon_pickaxe': {
        'name'               : 'pickaxe',
        'category'           : 'weapon',
        'subtype'            : 'axe',
        'damage_types'       : ['stabbing'],
        'char'               : 'T',
        'color'              : libtcodpy.yellow,
        'type'               : 'item',
        'slot'               : 'both hands',
        'description'        : 'A heavy digging implement used by miners. Can be used to dig through the walls '
                               'of the dungeon',
        'stamina_cost'       : 18,
        'str_requirement'    : 14,
        'pierce'             : 1,
        'shred'              : 1,
        'accuracy'           : -3,
        'ctrl_attack'        : item_actions.pickaxe_dig,
        'ctrl_attack_desc'   : 'Dig - dig through walls in this direction.',
        'weapon_dice'        : '1d8',
        'str_dice'           : 3,
        'attack_delay'       : 28,
        'crit_bonus'         : 1.5
    },
    # Maces
    'weapon_club': {
        'name'               : 'club',
        'category'           : 'weapon',
        'subtype'            : 'mace',
        'damage_types'       : ['bludgeoning'],
        'char'               : chr(157),
        'color'              : libtcodpy.yellow,
        'type'               : 'item',
        'slot'               : 'right hand',
        'description'        : 'A blunt piece of wood used for smashing things. The primitive weapon of brutes.',
        'stamina_cost'       : 10,
        'str_requirement'    : 6,
        'shred'              : 1,
        'accuracy'           : 2,
        'weapon_dice'        : '1d10',
        'str_dice'           : 2,
        'attack_delay'       : 20,
        'crit_bonus'         : 1.5,
        'allowed_materials'  : ['wooden']
    },
    'weapon_mace': {
        'name'               : 'mace',
        'category'           : 'weapon',
        'subtype'            : 'mace',
        'damage_types'       : ['bludgeoning'],
        'char'               : chr(157),
        'color'              : libtcodpy.yellow,
        'type'               : 'item',
        'slot'               : 'right hand',
        'description'        : 'A one-handed flanged mace. Good against armored enemies',
        'stamina_cost'       : 10,
        'str_requirement'    : 10,
        'shred'              : 2,
        'accuracy'           : 2,
        'weapon_dice'        : '1d18',
        'str_dice'           : 3,
        'on_hit'             : ['on_hit_stun'],
        'attack_delay'       : 18,
        'crit_bonus'         : 1.5
    },
    'weapon_warhammer': {
        'name'               : 'warhammer',
        'category'           : 'weapon',
        'subtype'            : 'mace',
        'damage_types'       : ['bludgeoning'],
        'char'               : chr(157),
        'color'              : libtcodpy.yellow,
        'type'               : 'item',
        'slot'               : 'both hands',
        'description'        : 'A heavy hammer head mounted to a haft. Made for slaying armored foes outright.',
        'stamina_cost'       : 15,
        'str_requirement'    : 18,
        'shred'              : 2,
        'pierce'             : 3,
        'accuracy'           : 1,
        'weapon_dice'        : '1d32',
        'str_dice'           : 6,
        'on_hit'             : ['on_hit_stun'],
        'attack_delay'       : 20,
        'crit_bonus'         : 1.5
    },
    'weapon_coal_mace': {
        'name'               : 'coal-brazer mace',
        'category'           : 'weapon',
        'damage_types'       : ['bludgeoning', 'fire'],
        'subtype'            : 'mace',
        'char'               : chr(157),
        'color'              : libtcodpy.yellow,
        'type'               : 'item',
        'slot'               : 'right hand',
        'description'        : 'A one-handed mace, fitted with a grate for burning coals.',
        'stamina_cost'       : 10,
        'str_requirement'    : 10,
        'shred'              : 2,
        'accuracy'           : 3,
        'weapon_dice'        : '1d22',
        'str_dice'           : 3,
        'on_hit'             : ['on_hit_stun'],
        'attack_delay'       : 18,
        'crit_bonus'         : 1.5
    },
    # Crossbows
    'weapon_crossbow': {
        'name'              : 'crossbow',
        'category'          : 'weapon',
        'damage_types'      : ['stabbing'],
        'subtype'           : 'ranged',
        'char'              : chr(23),
        'color'             : libtcodpy.yellow,
        'type'              : 'item',
        'slot'              : 'left hand',
        'description'       : "A crossbow, made so it can be worn over the left forearm. Can't be "
                              "reloaded during battle.",
        'str_requirement'   : 8,
        'shred'             : 2,
        'garunteed_shred'   : 1,
        'accuracy'          : 20,
        'weapon_dice'       : '1d20',
        'str_dice'          : 0,
        'attack_delay'      : 1000,
        'crit_bonus'        : 1.5,
        'range'             : 10,
        'ability'           : 'ability_off_hand_shot',
        'attributes'        : ['no_melee']
    },
    # Bows
    'weapon_longbow': {
        'name'              : 'longbow',
        'category'          : 'weapon',
        'damage_types'      : ['stabbing'],
        'subtype'           : 'ranged',
        'char'              : 'D',
        'color'             : libtcodpy.yellow,
        'type'              : 'item',
        'slot'              : 'both hands',
        'description'       : 'A longbow. It has excellent range but takes time to pull back.',
        'str_requirement'   : 10,
        'shred'             : 2,
        'garunteed_shred'   : 0,
        'accuracy'          : 20,
        'weapon_dice'       : '1d32',
        'str_dice'          : 1,
        'attack_delay'      : 1000,
        'crit_bonus'        : 1.5,
        'range'             : 20,
        'ability'           : 'ability_bow_shot',
        'attributes'        : ['no_melee']
    },
    'weapon_shortbow': { #todo - rework bows
        'name'              : 'longbow',
        'category'          : 'weapon',
        'damage_types'      : ['stabbing'],
        'subtype'           : 'ranged',
        'char'              : 'D',
        'color'             : libtcodpy.yellow,
        'type'              : 'item',
        'slot'              : 'both hands',
        'description'       : 'A shortbow. It has short range, but can be fired more quickly than a larger weapon.',
        'str_requirement'   : 9,
        'shred'             : 2,
        'garunteed_shred'   : 0,
        'accuracy'          : 20,
        'weapon_dice'       : '1d32',
        'str_dice'          : 1,
        'attack_delay'      : 1000,
        'crit_bonus'        : 2.0,
        'range'             : 8,
        'ability'           : 'ability_bow_shot',
        'attributes'        : ['no_melee']
    },
    # Misc
    'weapon_boomerang': {
        'name'               : 'boomerang',
        'category'           : 'weapon',
        'subtype'            : 'dagger',
        'damage_types'       : ['slashing'],
        'char'               : '>',
        'color'              : libtcodpy.yellow,
        'type'               : 'item',
        'slot'               :'right hand',
        'description'        : 'An exotic weapon from far away. It can be thrown and, with skill, caught again.',
        'stamina_cost'       : 6,
        'str_requirement'    : 8,
        'shred'              : 1,
        'accuracy'           : 4,
        'weapon_dice'        : '1d26',
        'str_dice'           : 1,
        'attack_delay'       : 12,
        'crit_bonus'         : 1.5,
        'ability'            : 'ability_boomerang',
    },
}